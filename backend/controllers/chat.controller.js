...
exports.sendCustomerMessage = async (req, res) => {
  try {
    ...
    const customerUser = await User.findById(thread.customerId).select('name');
    await writeThreadSnapshot(thread._id.toString(), {
      customerId: thread.customerId.toString(),
      customerName: customerUser?.name || 'Unknown Customer',
...
exports.getThreadData = async (req, res) => {
  try {
    ...
    // Check authorization
    if (req.user.role === 'customer' && thread.customerId._id.toString() !== req.user._id.toString()) {
    ...
    if (req.user.role === 'admin' && thread.adminId._id.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        status: 'error',
        message: 'Access denied'
      });
    ...
    const threadWithName = {
      ...thread.toObject(),
      customerName: thread.customerId?.name || 'Unknown Customer'
    };

    return res.status(200).json({
      status: 'success',
      data: { thread: threadWithName }
    });
  } catch (error) {
    console.error('Get thread data error:', error);
    res.status(500).json({
      status: 'error',
      message: 'Error fetching thread data'
    });
  }
};
...
exports.getAdminThreads = async (req, res) => {
  try {
    ...
    // Map threads to include customerName for easier access
