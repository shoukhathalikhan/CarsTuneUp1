...
export default function VehicleSelectionScreen({ navigation, route }) {
  const { fromProfile = false, fromHome = false, isFirstTimeSetup = false } = route.params || {};
  ...
  const handleModelSelect = async (model) => {
    ...
    try {
      ...
      // If this is first-time setup, mark profile as complete
      if (isFirstTimeSetup) {
        try {
          await api.put('/users/profile', { isProfileSetupComplete: true });
        } catch (error) {
        ...
        [
          {
            ...
            onPress: () => {
              if (fromProfile) {
                ...
                // First-time setup complete, go to home
