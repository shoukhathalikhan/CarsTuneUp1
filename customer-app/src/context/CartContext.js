...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (service, selectedAddOns = {}) => {
    // Convert selectedAddOns object to array, filtering out any invalid entries
    const addOnsArray = Object.values(selectedAddOns).filter(addon => 
      addon && addon._id && typeof addon.price !== 'undefined'
    ).map(addon => ({
      id: addon._id,
    ...
    const addOnsTotal = addOnsArray.reduce((sum, addon) => sum + (Number(addon.price) || 0), 0);
